...
A simpler variant of [decoders](decoders) is the mnemonic translator.
This can convert bit-vector values into text using a simple configuration file.

The configuration files are located either in `.surfer/mnemonic/` in the current working directory or in any of the following, OS-dependent, directories:
