...
/// Parse a line and extract the variable value, label, value kind, and length of the value.
...
/// Parse the first item into a VariableValue and its bit length.
fn parse_key_and_kind(token: &str) -> Result<(VariableValue, u32), MnemonicParseError> {
    ...
    if cleaned.starts_with("0o") || cleaned.starts_with("0O") {
        ...
        // We do not want a multiple of 3 here, but the actual bit length
    ...
    if cleaned.starts_with("0x") || cleaned.starts_with("0X") {
        ...
        // We do not want a multiple of 4 here, but the actual bit length
        let bits = num.bits() as u32;
        return Ok((VariableValue::BigUint(num), bits));
    }
