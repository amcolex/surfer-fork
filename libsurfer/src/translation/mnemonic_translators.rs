...
#[derive(Debug, Clone)]
/// A mapping of variable values to their mnemonic entries.
struct MnemonicMap {
    name: String,
    bits: u32,
...
/// Parse a file and return a MnemonicMap.
...
/// Parse content of file and return a MnemonicMap, falling back to default_name if no name is found in the file.
fn parse_content_with_default_name(
    ...
    for (line_num, line_str) in content.lines().enumerate() {
        ...
        if let Some(bits_value) = parse_specifier(processed, "bits") {
            ...
            let bits_num = bits_value
                .parse::<u32>()
                .map_err(|_| MnemonicParseError::InvalidBitsValue(bits_value.to_string()))?;
            bits = Some(bits_num);
            continue;
        }
    ...
    let final_name = name
        .or(default_name)
        .unwrap_or_else(|| "Unknown mnemonic".to_string());
    ...
    match value {
        ...
        VariableValue::String(s) => {
            ...
            if s.chars().all(|c| c == '0' || c == '1') {
                ...
                let extended = format!(
                    "{extra}{body}",
                    extra = extend_string(s, bit_width as u32),
                    body = s
                );
...
fn parse_line(line: &str) -> Result<(VariableValue, String, ValueKind, u32), MnemonicParseError> {
    ...
    if remainder.is_empty() {
        return Err(MnemonicParseError::MissingMnemonic);
    }
...
fn parse_key_value(token: &str) -> Result<(VariableValue, u32), MnemonicParseError> {
    ...
    Ok((VariableValue::String(lower.to_string()), lower.len() as u32))
}
...
mod tests {
    ...
    fn error_variants_coverage() {
        ...
        match parse_line("0101") {
            Err(MnemonicParseError::MissingMnemonic) => {}
