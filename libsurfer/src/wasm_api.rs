...
pub(crate) static MESSAGE_QUEUE: std::sync::LazyLock<Mutex<Vec<Message>>> =
    std::sync::LazyLock::new(|| Mutex::new(vec![]));

static QUERY_QUEUE: std::sync::LazyLock<tokio::sync::Mutex<VecDeque<Callback>>> =
    std::sync::LazyLock::new(|| tokio::sync::Mutex::new(VecDeque::new()));

// TODO: Let's make these take CXXRTL messages instead of strings
pub(crate) static CXXRTL_SC_HANDLER: std::sync::LazyLock<IngressHandler<String>> =
    std::sync::LazyLock::new(IngressHandler::new);

pub(crate) static CXXRTL_CS_HANDLER: std::sync::LazyLock<GlobalChannelTx<String>> =
