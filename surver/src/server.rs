...
fn get_info_page(shared: Arc<ReadOnly>, state: Arc<RwLock<SurverState>>) -> String {
    ...
    <tr><th>Filename</th><th>Load Progress</th><th>Last Modification Time</th><th>Last Reload Time</th></tr>
    {}
    </table>
    </body></html>
    "#,
...
fn get_status(state: Arc<RwLock<SurverState>>, file_index: usize) -> Result<Vec<u8>> {
    let state_guard = state.read().expect("State lock poisoned in get_status");
    let file_info = &state_guard.file_infos[file_index];
    let surver_file_info = SurverFileInfo {
    ...
    let status = SurverStatus {
        ...
        file_infos: vec![surver_file_info],
    };
...
async fn handle_cmd(
    ...
    let response = match (cmd, args) {
        ("get_status", []) => {
            let body = get_status(state, file_index)?;
            Response::builder()
...
async fn handle(
    ...
    let response = if let Some(cmd) = path_parts.get(1) {
        handle_cmd(state, &txs, cmd, 0, &path_parts[2..]).await?
    } else {
